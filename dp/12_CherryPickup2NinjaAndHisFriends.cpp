...
int main() {
	// your code goes here
// Cherry pickup 2 ninja and his friends 3d dp last video on dp on grids
// recursion
// TC->O(3^N * 3^N) SC->O(N) auxiliary stack space recursion
}
...
int maximumchocolate(int n,int m,vector<vector<int>>&grid)
{
    ...
    for(int j1=0;j1<m;j1++)
    {
        for(int j2=0;j2<m;j2++)
        {
            ...
            else d[n-1][j1][j2]=grid[n-1][j1]+grid[n-1][j2];
        }
    }
    int i;
    for(i=n-2;i>=0;i--)
